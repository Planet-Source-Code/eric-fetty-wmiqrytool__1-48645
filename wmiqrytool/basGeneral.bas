...
Public Sub SaveDefaultTable()
    If frmDB.cmbTables.Text <> DEF_CUSTOM_SQL Then
        If LastOpenedType = e_LastOpened_Access Then
            Call SaveSetting(App.Title, DEF_REGISTRY_CONNECTIONS & "\" & DEF_ACCESS, "Default Table", frmDB.cmbTables.Text, HKEY_LOCAL_MACHINE)
        Else
            If dbType = e_databaseTypes_OracleMSDA Then
                Call SaveSetting(App.Title, DEF_REGISTRY_CONNECTIONS & "\" & DEF_ORACLE_CLIENT, "Default Table", frmDB.cmbTables.Text, HKEY_LOCAL_MACHINE)
            ElseIf dbType = e_databaseTypes_OracleODBC Then
                Call SaveSetting(App.Title, DEF_REGISTRY_CONNECTIONS & "\" & DEF_ORACLE_ODBC, "Default Table", frmDB.cmbTables.Text, HKEY_LOCAL_MACHINE)
            ElseIf dbType = e_databaseTypes_SQLserver Then
                Call SaveSetting(App.Title, DEF_REGISTRY_CONNECTIONS & "\" & DEF_SQL_SERVER, "Default Table", frmDB.cmbTables.Text, HKEY_LOCAL_MACHINE)
            ElseIf dbType = e_databaseTypes_DSNFile Then
                Call SaveSetting(App.Title, DEF_REGISTRY_CONNECTIONS & "\" & DEF_DSN_FILE, "Default Table", frmDB.cmbTables.Text, HKEY_LOCAL_MACHINE)
            End If
        End If
    End If
End Sub
